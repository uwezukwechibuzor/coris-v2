...
function UndelegationsContent(props) {
  ...
  const unDelegationsData =
    props?.unbonding_responses !== undefined
      ? props?.unbonding_responses?.map((delegator) => {
          return delegator;
        })
      : null;
  ...
  const currentunDelegationsData = unDelegationsData?.slice(
    offset,
    offset + PER_PAGE
  );
              ...
              {currentunDelegationsData.length !== 0 ? (
                currentunDelegationsData?.map((data) => (
                  <tr className="striped">
                    ...
                    <td>{data ? data.entries.length : null}</td>
                    <td>
                      {data?.entries
                        ? toDay(
