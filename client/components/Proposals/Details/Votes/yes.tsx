...
function YesVoteOptions(props) {
  const [currentPage, setCurrentPage] = useState(0);

  const yesVotes = props.voteOptionYes;

  //add pagination to signatures
  const PER_PAGE = 15;
  const offset = currentPage * PER_PAGE;
  const currentActiveYesVotes = yesVotes?.slice(offset, offset + PER_PAGE);
  const pageCount = Math.ceil(yesVotes?.length / PER_PAGE);
  function handlePageClick({ selected: selectedPage }) {
    setCurrentPage(selectedPage);
  }

  let tBodyData = [];
  for (const i in currentActiveYesVotes) {
    tBodyData.push(
      <tbody>
        <tr className="striped">
            ...
            <Flex style={{ alignItems: "center" }}>
              ...
              <div className="ml-3">
                <Link
                  ...
                  as={`/account/${currentActiveYesVotes[i].voter}`}
                >
          ...
          <td>
            {currentActiveYesVotes[i]?.option === "VOTE_OPTION_YES"
              ? "Yes"
              : null}
          </td>
        </tr>
  ...
  return (
    <>
      <div>
        ...
        {currentActiveYesVotes?.length !== 0 ? (
          <ReactPaginate
            previousLabel={"←"}
            nextLabel={"→"}
            pageCount={pageCount}
