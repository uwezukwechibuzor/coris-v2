...
function AbstainVoteOptions(props) {
  ...
  const abstainVotes = props.voteOptionAbstain;

  //add pagination to signatures
  const PER_PAGE = 15;
  const offset = currentPage * PER_PAGE;
  const currentActiveAbstainVotes = abstainVotes?.slice(
    offset,
    offset + PER_PAGE
  );
  const pageCount = Math.ceil(abstainVotes?.length / PER_PAGE);
  function handlePageClick({ selected: selectedPage }) {
    setCurrentPage(selectedPage);
  }

  let tBodyData = [];
  for (const i in currentActiveAbstainVotes) {
    tBodyData.push(
      <tbody>
        <tr className="striped">
          ...
          <td>
            {currentActiveAbstainVotes[i]?.option === "VOTE_OPTION_ABSTAIN"
              ? "Abstain"
              : null}
          </td>
        </tr>
  ...
  return (
    <>
      <div>
        ...
        {currentActiveAbstainVotes?.length !== 0 ? (
          <ReactPaginate
            previousLabel={"←"}
            nextLabel={"→"}
            pageCount={pageCount}
