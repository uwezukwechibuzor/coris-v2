...
function NoVoteOptions(props) {
  const [currentPage, setCurrentPage] = useState(0);

  const noVotes = props.voteOptionNo;

  //add pagination to signatures
  const PER_PAGE = 15;
  const offset = currentPage * PER_PAGE;
  const currentActiveNoVotes = noVotes?.slice(offset, offset + PER_PAGE);
  const pageCount = Math.ceil(noVotes?.length / PER_PAGE);
  function handlePageClick({ selected: selectedPage }) {
    setCurrentPage(selectedPage);
  }

  let tBodyData = [];
  for (const i in currentActiveNoVotes) {
    tBodyData.push(
      <tbody>
        <tr className="striped">
            ...
            <Flex style={{ alignItems: "center" }}>
              ...
              <div className="ml-3">
                <Link
                  ...
                  as={`/account/${currentActiveNoVotes[i].voter}`}
                >
          ...
          <td>
            {currentActiveNoVotes[i]?.option === "VOTE_OPTION_NO" ? "No" : null}
          </td>
        </tr>
      </tbody>
    );
