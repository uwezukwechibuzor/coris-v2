...
function AccountDelegationsContent(props) {
  ...
  //add pagination to delegations
  const [currentPage, setCurrentPage] = useState(0);

  const PER_PAGE = 5;
  const offset = currentPage * PER_PAGE;
  const currentdelegations = delegations?.delegation_responses?.slice(
    offset,
    offset + PER_PAGE
  );
  const pageCount = Math.ceil(
    delegations?.delegation_responses?.length / PER_PAGE
  );
                  ...
                  <tr className="striped">
                      ...
                      <Flex style={{ alignItems: "center" }}>
                        ...
                        <div className="ml-3">
                          <Link
                            ...
                            as={`/${props?.chain_id}/validators/${data.delegation.validator_address}`}
                          >
                            <a>
                    ...
                    <td>
                      <Link
                        ...
                        as={`/${props?.chain_id}/account/${data.delegation.delegator_address}`}
                      >
                        <a>
                    ...
                    <td>
                      {data?.balance?.amount
                        ? (data.balance.amount / DENOM).toFixed(4)
                        : 0}{" "}
