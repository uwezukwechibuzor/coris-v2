...
chainValidatorDelegations?.delegation_responses?.map((delegator) => {
  const delegatorsAddress = delegator?.delegation?.delegator_address

  const getChainValidatorDelegations = await fetch(`https:${chainValidatorReDelegationsEndpoint(delegatorsAddress)}`);
  const chainValidatorDelegations = await getChainValidatorDelegations.json();
