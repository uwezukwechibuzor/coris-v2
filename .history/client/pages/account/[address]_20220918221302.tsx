...
export async function getServerSideProps({ res, req }) {
    ...
    try {
     ...
     const authAccount =  getAuthAccount !== undefined await getAuthAccount.json()
    
     //get account balance
     const getAccountBalance  = isServerReq(req) ? await fetch(`https:${accountBalanceEndpoint}`) : null
     const accountBalance = getAccountBalance !== undefined? await getAccountBalance.json() : {}
     ...
     const accountDelegatorRewards =  getAccountDelegatorRewards !== undefined? await getAccountDelegatorRewards.json() : {}
    
     //get delegations
     const  getAccountDelegations  = isServerReq(req) ? await fetch(`https:${accountDelegationsEndpoint}`) : null
     const  accountDelegations = getAccountDelegations !== undefined? await  getAccountDelegations.json() : {}
     
     
     //get Redelegations
     const  getAccountRedelegations  = isServerReq(req) ? await fetch(`https:${accountReDelegationsEndpoint}`) : null
     const  accountRedelegations = getAccountRedelegations !== undefined? await  getAccountRedelegations.json() : {}  //get Redelegations
     
     
     const  getAccountUnDelegations  = isServerReq(req) ? await fetch(`https:${accountUnDelegationsEndpoint}`) : null
     const  accountUnDelegations  =  getAccountUnDelegations !== undefined ? await  getAccountUnDelegations .json() : {}
