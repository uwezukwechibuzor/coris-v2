...
function AccountDelegationsContent(props) {
    ...
    delegations?.delegation_responses?.map(data => {
        console.log(data.delegation.delegator_address)
        console.log(data.delegation.validator_address)
        console.log(data.delegation.shares)
    })
                                                ...
                                                <tr className="striped">
                                                    <td>
                                                        <Flex style={{ alignItems: 'center' }}>
                                                            ...
                                                            <div className="ml-3">{data?.delegation?.validator_address ? formatHash(data.delegation.validator_address, 10, '.....') : null}</div>
                                                        </Flex>
                                                    </td>
                                                    <td>
                                                    {data?.delegation?.shares ? data.delegation.shares : null}
                                                    </td>
                                                    <td>
                                                    {data?.delegation?.delegator_address ? formatHash(data.delegation.validator_address, 10, '.....') : null}
                                                    </td>
                                                    <td>
                                                        1000
                                                    </td>
