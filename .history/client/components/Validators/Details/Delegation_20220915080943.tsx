...
function DelegationsContent(props) {
  console.log(props)
    const delegatorsData = props?.delegation_responses !== undefined? props?.data?.delegation_responses?.map((delegator) => {
        return delegator
    }) : null
                      ...
                      <Link href='/account[address]' as={`/account/${data.delegation.delegator_address}`} ><a>
                        <td>
                          <Flex style={{ alignItems: 'center' }}>
                            ...
                            <div className="ml-3">{data.delegation? formatHash(data.delegation.delegator_address, 15, '...') : null}</div>
                          </Flex>
                        </td>
                         </a></Link>
                        <td>
                          {data.balance? data.balance.amount : null} {data.balance? data.balance.denom : null} 
                        </td>
                      </tr>
                    </tbody>
                    )}
