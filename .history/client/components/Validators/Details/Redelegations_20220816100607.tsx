...
const validators = getAllValidators.isLoading === false? getAllValidators.data.validators : null
 const validator = validators?.filter((validator, index) => {
      if (validator?.operator_address.includes(operatorAddress)){
           return validator
      }
 })
 return validator?.map(data => {
   valDetails.push(data)
 })
 
  console.log(valDetails)

}    

const y = getValidatorNameByAddress('cosmosvaloper1sjllsnramtg3ewxqwwrwjxfgc4n4ef9u2lcnj0')
console.log(y)
