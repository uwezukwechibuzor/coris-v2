...
function RedelegationsContent(props) {
     ...
     let  getRedelegators
     let redelegation = []
     let getValidatorNameByAddress: function name(params:type) {
    ...
    getRedelegators?.map(redelegatorData => {
     ...
     getValidatorNameByAddress = (operatorAddress: string) => {
      let valName
    ...
    const validators = getAllValidators.isLoading === false? getAllValidators.data.validators : null
     validators?.filter((validator, index) => {
          if (validator?.operator_address.includes(operatorAddress)){
                valName = validator.description.moniker
          }
     })
     return valName
    }   
...
const u = getValidatorNameByAddress('cosmosvaloper1sjllsnramtg3ewxqwwrwjxfgc4n4ef9u2lcnj0')
console.log(u)
                  ...
                  {redelegation?.map(data => 
                    <tbody>
                      ...
                      <Link href='/account[address]' as={`/account/${data.redelegation.delegator_address}`} ><a>
                        ...
                        <td>
                         {data.redelegation? getValidatorNameByAddress(data.redelegation?.validator_src_address) : null}
                        </td>
                        <td>
                          sdfdfdsfsdfsdffsdf
                        </td>
