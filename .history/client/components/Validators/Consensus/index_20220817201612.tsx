            ...
            <Responsive>
              <table className="w-100 mt-3 table table-responsive">
                ...
                {consensus?.map((details, index) => {
                        ...
                        if (data !== undefined) {
                            //console.log(data[0])
                      var percentageOfVotingPower: number = getPercentageOfValidatorsBondedTokens(data[0]?.tokens, totalBondedTokens)
                   console.log()
                      activeValidatorsCumulativeShare += percentageOfVotingPower
                      
                      return (
                        <tr className="validator-item-row" onClick={() => router.push(`/validators/${data[0].operator_address}`)} >
                          <td>{i + 1}</td>
                          <td>
                            <Flex>
                              <FlexMiddle>
                                <img className="img" src={data? getValidatorsLogoFromWebsites(data[0]?.description?.website): null} alt="" />
                              </FlexMiddle>
                              <FlexMiddle>
                                {data[0]?.description?.moniker}
                              </FlexMiddle>
                            </Flex>
                          </td>
                          <td>
                            {data? roundValidatorsVotingPowerToWholeNumber(data[0]?.tokens): 0}
                            <div style={{color: 'red'}} className="sub">{data? percentageOfVotingPower.toFixed(2) + '%': 0}</div>
                          </td>
                          <td>{data? activeValidatorsCumulativeShare.toFixed(2) + '%' : 0}</td>
                          <td>{data[1]}</td>
                          <td>{</td>
                        </tr>
                     )
