...
return preCommit
  ...
  const validatorsConsensusDteails = consensus?.filter((data, i )=> {
     const y= data.map(details => {
    if (query === 'binance') {
        return details;
      } else if (details[0]?.description?.moniker.toLowerCase().includes(query.toLocaleLowerCase())) {
        return details
      }
     })
     return y[i]
  })
   console.log(validatorsConsensusDteails)
          ...
          <Tab eventKey="active" title="Active" className="w-100">
            <Responsive>
              <table className="w-100 mt-3 table table-responsive">
                ...
                <tbody>
                {consensus?.filter(data => {
                    const getDetails = data.map(details => {
                    if (query === 'binance') {
                        return details;
                    } else if (details[0]?.description?.moniker.toLowerCase().includes(query.toLocaleLowerCase())) {
                        return details
                 ...
                 .map((getDetails, index) => {
                     var percentageOfVotingPower: number = getPercentageOfValidatorsBondedTokens(getDetails?.tokens, totalBondedTokens)
                   //console.log(getDetails[0])
                     activeValidatorsCumulativeShare += percentageOfVotingPower
                      
                      return (
                        <tr className="validator-item-row" onClick={() => router.push(`/validators/${getDetails.operator_address}`)} >
                          <td>{index + 1}</td>
                          <td>
                            <Flex>
                              <FlexMiddle>
                                <img className="img" src={getValidatorsLogoFromWebsites(getDetails?.description?.website)} alt="" />
                              </FlexMiddle>
                              <FlexMiddle>
                                {getDetails?.description?.moniker}
                              </FlexMiddle>
                            </Flex>
                          </td>
                          <td>
                            {roundValidatorsVotingPowerToWholeNumber(getDetails?.tokens)}
