            ...
            <Responsive>
              <table className="w-100 mt-3 table table-responsive">
                ...
                {consensus?.map((details, index) => {
                        ...
                        if (data !== undefined) {
                            //console.log(details[index][0])
                      var percentageOfVotingPower: number = getPercentageOfValidatorsBondedTokens(details[index][0]?.tokens, totalBondedTokens)

                      activeValidatorsCumulativeShare += percentageOfVotingPower
                      
                      return (
                        <tr className="validator-item-row" onClick={() => router.push(`/validators/${details[index][0].operator_address}`)} >
                          <td>{i + 1}</td>
                          <td>
                            <Flex>
                              <FlexMiddle>
                                <img className="img" src={data? getValidatorsLogoFromWebsites(details[index][0]?.description?.website): null} alt="" />
                              </FlexMiddle>
                              <FlexMiddle>
                                {details[index][0]?.description?.moniker}
                              </FlexMiddle>
                            </Flex>
                          </td>
                          <td>
                            {details[index][0]?.tokens? roundValidatorsVotingPowerToWholeNumber(details[index][0]?.tokens): 0}
                            <div style={{color: 'red'}} className="sub">{details[index][0]?.tokens? percentageOfVotingPower.toFixed(2) + '%': 0}</div>
                          </td>
                          <td>{details[index][0]?.tokens? activeValidatorsCumulativeShare.toFixed(2) + '%' : 0}</td>
                          <td>{details[index][1] === 'nil-Vote'? <img src="https://img.icons8.com/color/20/000000/cancel--v1.png"/> : <img src="https://img.icons8.com/fluency/20/000000/ok.png"/> }</td>
                          <td>{details[index][2] === 'nil-Vote'? <img src="https://img.icons8.com/color/20/000000/cancel--v1.png"/> : <img src="https://img.icons8.com/fluency/20/000000/ok.png"/> }</td>
                        </tr>
                     )
                    }
                  })
