...
function ConsensusDetails(props) {
  ...
  consensusState?.result?.round_state?.height_vote_set.map((data, index) => {
            
        data?.prevotes.map((details, i) => preVotes.push(details[index]))

        data?.precommits?.map((details, i ) => {
           preCommit.push(details[index])
       })
  })
