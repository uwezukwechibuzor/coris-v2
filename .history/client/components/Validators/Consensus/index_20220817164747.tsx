...
function ConsensusDetails(props) {
  ...
  const y =  consensusState?.result?.round_state?.height_vote_set.map((data) => {
    const pre = data?.prevotes.map((details, i) => {
        return [activeVal[i], details] 
    })
    
    const preCommit = data?.precommits?.map(details => {
