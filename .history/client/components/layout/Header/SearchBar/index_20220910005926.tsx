  ...
  searchingValidators?.filter(data => {
    if (query === '') {
      ...
      err = 'No Data Found';
       ...
       data?.operator_address === query? searchedData = <a href={`/validators/${data.operator_address}`}>{data?.operator_address}</a> : searchedData = <a href={`/validators/${data.operator_address}`}>{data?.operator_address}</a>
    }
  })
