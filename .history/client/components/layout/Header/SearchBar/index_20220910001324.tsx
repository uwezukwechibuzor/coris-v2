  ...
  searchingValidators?.filter(data => {
   ...
   // }else if(query !== data?.description?.moniker){
      ...
      searchedData = data?.operator_address
       ...
       searchedDataa = data?.description?.moniker
    }
  })
              ...
              <ListGroup className="w-100">
                {
                  searchResult?.map((item) => (
                    <ListGroup.Item key={item?.id} className={darkMode ? "dark-mode text-white no-border text-white" : "no-border"}>{searchedData || err || searchedDataa }</ListGroup.Item>
                  ))
                }
              </ListGroup>
            </SearchResult>
