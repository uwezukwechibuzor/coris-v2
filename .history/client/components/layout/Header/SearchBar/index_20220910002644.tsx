...
export function SearchBar(props) {
  ...
  let searchedValidatorAddress
  let searchedValidatorName
  let err
  searchingValidators?.filter(data => {
    if (query === '') {
      ...
      err = 'No Data Found';
   // }else if(query !== data?.description?.moniker){
      //err = 'No Data Found';
      ...
      searchedValidatorName = data?.description?.moniker;
      searchedValidatorAddress= data?.operator_address; 
    }
  })
              ...
              <ListGroup className="w-100">
                {
                  searchResult?.map((item) => (
                    <ListGroup.Item key={item?.id} className={darkMode ? "dark-mode text-white no-border text-white" : "no-border"}>{searchedValidatorAddress || searchedValidatorName || err  }</ListGroup.Item>
                  ))
                }
              </ListGroup>
            </SearchResult>
