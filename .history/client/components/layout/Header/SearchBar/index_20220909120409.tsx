...
export function SearchBar(props) {
 ...
 const searchingValidators = getAllChainValidators.isLoading === false? getAllChainValidators.data?.validators.map(data => data?.operator_address
  ...
  const searched = searchingValidators?.filter(data => {
    if (query === '') {
      console.log('no data')
...
console.log(searched)
  ...
  return (
    <SearchBar1 className={`search-bar ${className || ""} ${darkMode ? 'dark-mode' : ''}`}>
      ...
      <Card>
      This is some text within a card bodylllllllllllllllllllllllllllllllllllls is some text within a card bodyllllllllllllllllllllllllllllllllllllllllllThis is some text within a card bodylllllllllllllllllllllllllllllllllllls is some text within a card bodyllllllllllllllllllllllllllllllllllllllllll
    </Card>
    </SearchBar1>
  );
}
