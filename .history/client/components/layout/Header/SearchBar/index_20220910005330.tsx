  ...
  searchingValidators?.filter(data => {
    if (query === '') {
      ...
      err = 'No Data Found';
       ...
       data?.operator_address === query? searchedData = <p onClick={() => router.push(`/validators/${data.operator_address}`)} >{data?.operator_address}</p> : searchedData = data?.description?.moniker
    }
  })
