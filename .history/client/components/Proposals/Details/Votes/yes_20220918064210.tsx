...
function YesVoteOptions(props) {
  ...
  const yesVotes = props
 console.log(yesVotes)
  let tBodyData = []
  for (const i in yesVotes) {
    tBodyData.push(<tbody>
      <tr className="striped">
        <td>
          <Flex style={{ alignItems: 'center' }}>
            ...
            <div className="ml-3">
              <Link href='/account[address]' as={`/account/${yesVotes[i].voter}`} ><a>{yesVotes[i].voter}
              </a></Link>
            </div>
          </Flex>
        </td>
        <td>
          {yesVotes[i]?.option === 'VOTE_OPTION_YES' ? 'Yes' : null}
        </td>
      </tr>
    </tbody>)
  }
