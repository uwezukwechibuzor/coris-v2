         ...
         <table>
          ...
          <Grid>
            ...
            <Container>
              ...
              <div>{proposal? tallyPercentage(proposal?.proposal?.final_tally_result?.no).toFixed(2) + '%' : 0} ({proposal?.proposal?.final_tally_result?.no ? proposal?.proposal.final_tally_result.no.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") : 0} {proposal?.proposal.total_deposit ? proposal?.proposal.total_deposit[0].denom : ''})</div>
            </Container>
            <Container>
              <Flex>
