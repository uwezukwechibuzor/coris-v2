         ...
         <table>
          ...
          <Grid>
            <Container>
              ...
              <div>{proposal? tallyPercentage(proposal?.final_tally_result?.yes).toFixed(2) + '%' : 0} ({proposal?.proposal?.final_tally_result?.yes ? proposal?.proposal.final_tally_result.yes.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") : 0}  {proposal?.proposal?.total_deposit ? proposal?.proposal.total_deposit[0].denom : ''})</div>
            </Container>
            <Container>
              <Flex>
