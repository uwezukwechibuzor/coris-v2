...
function BlockHeightContent(props: any) {
  ...
  txs?.tx_responses !== null ? txs?.tx_responses?.map(tx => {
    ...
    const txsPER_PAGE = 10;
    const offsetTxs = currentTxsPage * txsPER_PAGE;
    const currentTxsData = txs?.tx_responses.slice(offset, offset + PER_PAGE) 
    const txspageCount = Math.ceil(validatorsSignaturesDetails?.length / txsPER_PAGE);
    function handleTxPageClick({ selected: selectedPage }) {
