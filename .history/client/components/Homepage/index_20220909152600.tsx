    ...
    <Wrapper className={darkMode ? 'dark-mode' : ''}>
      ...
      <Grid>
        <GridItem className={darkMode ? 'dark-mode first-item chart' : 'first-item chart'}>
          ...
          <Stat>
            {Math.sign(coinData?.market_data?.current_price?.usd) === 1? <Amount>${coinData?.market_data?.current_price?.usd}</Amount>: Math.sign(coinData?.market_data?.current_price?.usd) === -1? <Amount style={{color: 'red'}}>${coinData?.market_data?.current_price?.usd}</Amount> : nu }
            <Flex>
              <FlexCol>
                {/* <Increase>
                ...
                <Decrease>
                  <svg width="14" height="10" viewBox="0 0 14 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M0 10L7 0L14 10H0Z" fill={Math.sign(coinData?.market_data?.price_change_percentage_24h) === 1? '#2ec169' : 'red'} />
                  </svg>
                </Decrease>
              </FlexCol>
