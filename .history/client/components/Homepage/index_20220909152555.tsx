    ...
    <Wrapper className={darkMode ? 'dark-mode' : ''}>
      ...
      <Grid>
        <GridItem className={darkMode ? 'dark-mode first-item chart' : 'first-item chart'}>
          ...
          <Stat>
            {Math.sign(coinData?.market_data?.current_price?.usd) === 1? <Amount>${coinData?.market_data?.current_price?.usd}</Amount>: Math.sign(coinData?.market_data?.current_price?.usd) === -1? <Amount style={{color: 'red'}}>${coinData?.market_data?.current_price?.usd}</Amount> :  }
            <Flex>
              <FlexCol>
                {/* <Increase>
