...
let coinID = 'umee'
=======
let coinID = process.env.NEXT_PUBLIC_COIN_ID;
const denom = 1000000;
...
=======
  ...
  const joinedBlocksValidatorsData = getBlocks.map((block) => {
    //convert proposer address to cosmosvalcons
    const proposerToBech32 = toBech32("cosmosvalcons", fromHex(block.proposer));
    const getActiveChainValidators = getChainValidators?.data?.validators.map(
      (validator) => {
        //fetch just the active validators
        //get the consensus pubkey
        const ed25519PubkeyRaw = fromBase64(validator.consensus_pubkey.key);
...
>>>>>>> e1ac77f2ba93d38adfaf116cf65631f04cf1c9e1
      }
    );
...
=======
  ...
  const bondedTokens =
    getPool.isLoading == false
      ? (getPool?.data?.pool?.bonded_tokens / denom).toFixed(2)
      : null;
  const notBondedTokens =
    getPool.isLoading == false
      ? (getPool?.data?.pool?.not_bonded_tokens / denom).toFixed(2)
      : null;
  ...
  const totalValidators =
    getAllValidators.isLoading == false
      ? getAllValidators?.data?.validators?.length
      : null;
  const totalActiveValidators =
    getAllActiveValidators.isLoading == false
      ? getAllActiveValidators?.data?.validators?.length
      : null;
>>>>>>> e1ac77f2ba93d38adfaf116cf65631f04cf1c9e1

  //percentage of active Validators
  const percentageOfActiveValidators =
    totalActiveValidators != undefined && totalValidators != undefined
...
=======
  let totalVotingPower = 0;
  getAllValidators.isLoading == false
    ? getAllValidators?.data?.validators.map((validatorsDetails) => {
        totalVotingPower += Number(validatorsDetails.tokens / denom);
        return totalVotingPower;
      })
    : null;
  ...
  let totalActiveVotingPower = 0;
  getAllActiveValidators.isLoading == false
    ? getAllActiveValidators?.data?.validators.map((validatorsDetails) => {
        totalActiveVotingPower += Number(validatorsDetails.tokens / denom);
        return totalActiveVotingPower;
      })
    : null;
  ...
  const percentageOfVotingPower =
    getAllValidators.isLoading == false &&
    getAllActiveValidators.isLoading == false
      ? Math.round(Number(totalActiveVotingPower / totalVotingPower) * 100)
      : null;
>>>>>>> e1ac77f2ba93d38adfaf116cf65631f04cf1c9e1
...
=======
                      ...
                      <Link
                        ...
                        as={`/validators/${data.validator.operator_address}`}
                      >
                        <a>
                          <td>
                            <img
